...
public class ScalePeaceWidgetRenderer : ISkiaWidgetRenderer, IDisposable
{
    ...
    {
        var scaleBar = (ScalePeaceWidget)widget;
        ...
        (scaleBarLength, scaleBarText) = scaleBar.GetScaleBarLengthAndText(viewport);
        ...
        var points = scaleBar.GetScaleBarLinePositions(viewport, scaleBarLength, scaleBar.StrokeWidthHalo);
        ...
        if (!points.Any()) throw new NotImplementedException($"A {nameof(ScalePeaceWidget)} can not be drawn without line positions");
        ...
        scaleBarText ??= string.Empty;
        _paintScaleTextStroke.MeasureText(scaleBarText, ref textSize1);

        var (posX1, posY1) = scaleBar.GetScaleBarTextPosition(viewport, ToMRect(textSize1), scaleBar.StrokeWidthHalo);

        // Now draw text
        canvas.DrawText(scaleBarText, posX1, posY1 - textSize1.Top, _paintScaleTextStroke);
        canvas.DrawText(scaleBarText, posX1, posY1 - textSize1.Top, _paintScaleText);
    ...
    private MRect ToMRect(SKRect rect) => new MRect(rect.Left, rect.Top, rect.Right, rect.Bottom);
